...
if [ -n "${REMOTE_CONTAINERS}" ] || [ -n "${CODESPACES}" ] || [ -f "/.dockerenv" ]; then
    ...
    echo "--- Running in regular environment (Podman machine setup) ---"
...
if [ "$DEVCONTAINER_MODE" = true ]; then
    ...
    # Set APP_DIR to current directory for devcontainer
    APP_DIR="$(pwd)"

    echo "--- Running Ansible playbook 'init.yml' in devcontainer... ---"
