...
group_var() {
  sed -nE "s/^$1:[[:space:]]*\"?([^\"#]+)\"?.*/\1/p" "$GROUP_VARS" | head -1 | tr -d '[:space:]'
}
...
fetch() {
  ...
  curl -sfL -o "$2.part" "$1" || die "download failed: $1"
  mv "$2.part" "$2"
}
...
download_runtime() {
  ...
  fetch "https://github.com/kubernetes-sigs/cri-tools/releases/download/v${K8S_VERSION%.*}.0/crictl-v${K8S_VERSION%.*}.0-linux-$ARCH.tar.gz" "$crictl_tgz"
...
render_inline_kubelet_config() {
  python3 - "$REPO_ROOT/ansible/devcontainer-run.yml" "$1" <<'PY'
import re, sys
